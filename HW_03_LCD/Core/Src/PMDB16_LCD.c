...
uint8_t mena[8] = {0x00, 0x00, 0x00, 0x1F, 0x01, 0x01, 0x01, 0x1E};
uint8_t cing[8] = {0x00, 0x08, 0x12, 0x04, 0x08, 0x00, 0x00, 0x00};
uint8_t smiley[8] = {0x00, 0x00, 0x0A, 0x00, 0x11, 0x0E, 0x00, 0x00};
uint8_t heart[8] = {0x00, 0x00, 0x0A, 0x15, 0x11, 0x0E, 0x04, 0x00};
...
//  set (x = col, y = row) position of the cursor
...
void loadCustomChars(){ //write all custom characters to the LCD module memory
	writeCustomChar(0x01, mena);
	writeCustomChar(0x02, cing);
	writeCustomChar(0x03, smiley);
	writeCustomChar(0x04, heart);
...
void lcd_drawBar(int value){ //draws a bar using custom characters and spaces
	lcd_set_cursor(1,0); //bar is placed in the bottom row
